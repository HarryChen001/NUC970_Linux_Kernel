...
//added by mcuzone
//added by qhh use for am335x,not use for am1808
input_report_abs(ts->input_dev, ABS_PRESSURE,  (touch_num || key_value));
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
    ...
    //GTP_INFO("on  gtp_reset_guitar ");

    GTP_GPIO_AS_OUTPUT(GTP_INT_PORT);

   // (*(volatile unsigned int *)REG_GPIOD_DATAOUT)&=~(1<<7);
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
   ...
   // (*(volatile unsigned int *)REG_GPIOD_MFPL )&= 0X00FFFFFF;
  //  (*(volatile unsigned int *)REG_GPIOD_DIR)|=(1<<7);
 //   (*(volatile unsigned int *)REG_GPIOD_DATAOUT)|=(1<<7);
...
//added by qhh ref gt811,report_max_x->ts->abs_x_max ;report_max_y->ts->abs_y_max
